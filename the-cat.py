...
person_course_cleaned_china_path = os.path.join(topdir, PERSON_COURSE_DAY_CLEANED_CHINA)
person_course_day_cleaned_china_path = os.path.join(topdir, PERSON_COURSE_DAY_CLEANED_CHINA)

user_df = pd.read_table(user_path)
person_course_day_cleaned_df = pd.read_table(person_course_day_cleaned_path)
person_course_cleaned_china_df = pd.read_table(person_course_cleaned_china_path)
person_course_day_cleaned_china_df = pd.read_table(person_course_day_cleaned_china_path)

result = user_df.join(person_course_day_cleaned_df, lsuffix='user_id', rsuffix='user_id')
result_china = person_course_cleaned_china_df.join(person_course_day_cleaned_china_df, lsuffix='user_id', rsuffix='user_id')
...
#result_china = result_china.fillna(0)
...
#grades_china = result_china['grade']
#result_china = result_china.drop(['grade'], axis=1)
...
#result_norm_china = result_china.apply(lambda x: (x - np.mean(x)) / np.std(x))

result_norm.to_csv('result-norm.tsv', sep="\t")

frames = [result_norm, grades]
#frames_china = [result_norm_china, grades_china]


final_result = pd.concat(frames, axis=1)
#final_result_china = pd.concat(frames_china, axis=1)

final_result.to_csv('final-result.tsv', sep="\t")
#final_result_china.to_csv('final-result-china.tsv', sep="\t")
